...
void build_world(WORLD_World* world) {
    ...
    CUBE_Cube* front = CUBE_new();
    WORLD_add_object(world, front);
    MATERIAL_Material* front_mat = SPHERE_get_material(front);
    front_mat->ambient = 0;
//    front_mat->specular = 0;
    ...
    front_mat->shadow_calc = false;
    MATRIX_Matrix* front_rotation_x = MATRIX_new_rotation_x(M_PI_4);
    MATRIX_Matrix* front_rotation_z = MATRIX_new_rotation_z(M_PI_4);
    MATRIX_Matrix* front_translation = MATRIX_new_translation(-0.5, 1, -1.5);
    MATRIX_Matrix* front_scaling = MATRIX_new_scaling(.33, .33, .33);
    MATRIX_Matrix* front_transform = MATRIX_multiply_many(front_translation, front_scaling, front_rotation_x, front_rotation_z);
    CUBE_set_transform(front, front_transform);
    MATRIX_delete_all(front_translation, front_scaling, front_transform, front_rotation_x, front_rotation_z);
...
int main(void) {
    Try {
                ...
                const char *filename = "refraction_cube.ppm";
