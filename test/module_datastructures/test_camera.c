...
void test_transformation_matrix_for_default_orientation() {
    TUPLES_Point from = TUPLES_point(0, 0, 0);
    TUPLES_Point to = TUPLES_point(0, 0, -1);
...
void test_transformation_moves_the_world() {
    TUPLES_Point from = TUPLES_point(0, 0, 8);
    TUPLES_Point to = TUPLES_point(0, 0, 0);
    TUPLES_Vector up = TUPLES_vector(0, 1, 0);
...
void test_arbitrary_view_transformation() {
    TUPLES_Point from = TUPLES_point(1, 3, 2);
    TUPLES_Point to = TUPLES_point(4, -2, 8);
    TUPLES_Vector up = TUPLES_vector(1, 1, 0);
...
void test_get_ray_through_center_of_canvas() {
    ...
    TUPLES_Point origin_expected = TUPLES_point(0, 0, 0);
    TUPLES_Vector direction_expected = TUPLES_vector(0, 0, -1);
...
void test_get_ray_through_corner_of_canvas() {
    ...
    TUPLES_Point origin_expected = TUPLES_point(0, 0, 0);
    TUPLES_Vector direction_expected = TUPLES_vector(0.66519, 0.33259, -0.66851);
...
void test_get_ray_when_camera_is_transformed() {
    ...
    TUPLES_Point origin_expected = TUPLES_point(0, 2, -5);
    TUPLES_Vector direction_expected = TUPLES_vector(sqrt(2)/2.0, 0, -sqrt(2)/2.0);
...
void test_redering_world_with_a_camera() {
    ...
    TUPLES_Point from = TUPLES_point(0, 0, -5);
    TUPLES_Point to = TUPLES_point(0, 0, 0);
    TUPLES_Vector up = TUPLES_vector(0, 1, 0);
