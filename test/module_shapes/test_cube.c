...
void helper_check_local_intersect_t(double ox, double oy, double oz, double dx, double dy, double dz, double t1, double t2) {
...
void helper_check_local_no_intersect_t(double ox, double oy, double oz, double dx, double dy, double dz) {
  CUBE_Cube *cube = CUBE_new();
  RAY_Ray ray;
  RAY_init(&ray, ox, oy, oz, dx, dy, dz);
  RAY_Intersections *xs = RAY_new_intersections();
...
void helper_check_normal_at_point(double px, double py, double pz, double nx, double ny, double nz) {
  CUBE_Cube *cube = CUBE_new();
  TUPLES_Point p;
